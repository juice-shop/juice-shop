...
describe("CodeFixesService", () => {
  ...
  it("should get code fixes for challenge directly from the rest api", inject(
    [CodeFixesService, HttpTestingController],
    fakeAsync((service: CodeFixesService, httpMock: HttpTestingController) => {
      let res: any;
      service.get("testChallenge").subscribe((data) => (res = data));

      const req = httpMock.expectOne(
        "http://localhost:3000/snippets/fixes/testChallenge",
      );
  ...
  it('should submit solution for "Fit It" phase of coding challenge via the rest api', inject(
    [CodeFixesService, HttpTestingController],
    fakeAsync((service: CodeFixesService, httpMock: HttpTestingController) => {
      let res: any;
