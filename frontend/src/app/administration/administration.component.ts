  ...
  findAllUsers () {
    this.userService.find().subscribe((users) => {
      ...
      for (const user of this.userDataSource) {
        user.email = this.sanitizer.sanitize(`<span class="${this.doesUserHaveAnActiveSession(user) ? 'confirmation' : 'error'}">${user.email}</span>`)
      }
  ...
  findAllFeedbacks () {
    this.feedbackService.find().subscribe((feedbacks) => {
      ...
      for (const feedback of this.feedbackDataSource) {
        feedback.comment = this.sanitizer.sanitize(feedback.comment)
      }
