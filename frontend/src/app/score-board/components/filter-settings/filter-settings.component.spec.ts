...
describe("FilterSettingsComponent", () => {
  ...
  it("should emit difficulty filter change", () => {
    ...
    expect(component.filterSettingChange.emit).toHaveBeenCalledWith(
      jasmine.objectContaining({ difficulties }),
    );
  });
  ...
  it("should emit category filter change", () => {
    ...
    expect(component.filterSettingChange.emit).toHaveBeenCalledWith(
      jasmine.objectContaining({ categories }),
    );
  });
  ...
  it("should emit search query filter change", () => {
    ...
    expect(component.filterSettingChange.emit).toHaveBeenCalledWith(
      jasmine.objectContaining({ searchQuery }),
    );
  });
});
