...
export class ChallengesUnavailableWarningComponent implements OnChanges {
  ...
  public ngOnChanges() {
    const disabledChallenges = this.challenges.filter(
      (challenge) => challenge.disabledEnv !== null,
    );
    const disabledOnWindows = disabledChallenges.filter(
      (challenge) => challenge.disabledEnv === "Windows",
    );
