...
describe("filterChallenges", () => {
  it("should filter empty list", () => {
    ...
    expect(
      filterChallenges([], {
        categories: ["foo", "bar"],
        difficulties: [1, 2, 3, 5, 6],
        tags: ["hard"],
        status: "solved",
        searchQuery: "foobar",
        showDisabledChallenges: true,
        restrictToTutorialChallengesFirst: true,
      }),
    ).toEqual([]);
  });
  ...
  it("should filter challenges based on categories properly", () => {
    expect(
      filterChallenges([CHALLENGE_1, CHALLENGE_2, CHALLENGE_3], {
        ...DEFAULT_FILTER_SETTING,
        categories: ["foobar"],
      }).map((challenge) => challenge.key),
    ).toEqual(jasmine.arrayWithExactContents(["challenge-1", "challenge-2"]));
    expect(
      filterChallenges([CHALLENGE_1, CHALLENGE_2, CHALLENGE_3], {
        ...DEFAULT_FILTER_SETTING,
        categories: ["barfoo"],
      }).map((challenge) => challenge.key),
    ).toEqual(jasmine.arrayWithExactContents(["challenge-3"]));
  });
  ...
  it("should filter challenges based on difficulties properly", () => {
    ...
    ).toEqual(jasmine.arrayWithExactContents(["challenge-1", "challenge-3"]));
    expect(
      filterChallenges([CHALLENGE_1, CHALLENGE_2, CHALLENGE_3], {
        ...DEFAULT_FILTER_SETTING,
        difficulties: [3],
      }).map((challenge) => challenge.key),
    ).toEqual(jasmine.arrayWithExactContents(["challenge-2"]));
  });
  ...
  it("should filter challenges based on status properly", () => {
    ...
    ).toEqual(jasmine.arrayWithExactContents(["challenge-1"]));
    expect(
      filterChallenges([CHALLENGE_1, CHALLENGE_2, CHALLENGE_3], {
        ...DEFAULT_FILTER_SETTING,
  ...
  it("should only show tutorial challenges when restrictToTutorialChallengesFirst is set", () => {
    expect(
      filterChallenges(
        [
          CHALLENGE_1,
          { ...CHALLENGE_2, solved: false, difficulty: 1, tutorialOrder: null },
          CHALLENGE_3,
        ],
        { ...DEFAULT_FILTER_SETTING, restrictToTutorialChallengesFirst: true },
      ).map((challenge) => challenge.key),
  ...
  it("should only show unsolved tutorial of first difficulty and solved ones of easier difficulties", () => {
    expect(
      filterChallenges(
        [
          { ...CHALLENGE_1, solved: true },
          { ...CHALLENGE_2, solved: false },
          CHALLENGE_3,
        ],
        { ...DEFAULT_FILTER_SETTING, restrictToTutorialChallengesFirst: true },
      ).map((challenge) => challenge.key),
