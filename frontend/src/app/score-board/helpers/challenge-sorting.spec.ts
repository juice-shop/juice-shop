...
describe("sortChallenges", () => {
  it("sort tutorial challenges correctly", () => {
    expect(
      sortChallenges([CHALLENGE_1, CHALLENGE_2]).map(
        (challenge) => challenge.key,
      ),
    ).toEqual(["challenge-1", "challenge-2"]);
    expect(
      sortChallenges([CHALLENGE_2, CHALLENGE_1]).map(
        (challenge) => challenge.key,
      ),
    ).toEqual(["challenge-1", "challenge-2"]);
    for (const challengeOrderingCombination of [
      ...
      expect(
        sortChallenges(challengeOrderingCombination).map(
          (challenge) => challenge.key,
        ),
      ).toEqual(["challenge-1", "challenge-2"]);
    }
  ...
  it("sort non tutorial challenges by difficulty while still maintaining tutorial order for challenges with tutorials", () => {
    for (const challengeOrderingCombination of [
      ...
      ).toEqual(["challenge-1", "challenge-2", "challenge-3"]);
    }
  });
});
