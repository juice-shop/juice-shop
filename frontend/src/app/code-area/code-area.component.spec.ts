...
describe("CodeAreaComponent", () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [TranslateModule.forRoot(), CodeAreaComponent],
    }).compileComponents();
  });
  ...
  it("should generate unchecked markers for each line of the code snippet", () => {
    ...
    component.ngOnInit();
    expect(component.lineMarkers).toEqual([
      { lineNumber: 1, marked: false },
      { lineNumber: 2, marked: false },
      { lineNumber: 3, marked: false },
    ]);
  });

  it("should emit selected line of code when no lines were previously selected", () => {
    component.addLine = jasmine.createSpyObj(["emit"]);
    component.lineMarkers = [
      { lineNumber: 1, marked: false },
      { lineNumber: 2, marked: false },
      { lineNumber: 3, marked: false },
    ];
  ...
  it("should emit selected lines of code minus a deselected one", () => {
    ...
    component.lineMarkers = [
      { lineNumber: 1, marked: true },
      { lineNumber: 2, marked: true },
      { lineNumber: 3, marked: true },
    ];
    component.selectLines(2);
