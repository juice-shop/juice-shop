...
async function deleteUser(userId: number) {
  return await UserModel.destroy({ where: { id: userId } }).catch(
    (err: unknown) => {
      logger.error(
        `Could not perform soft delete for the user ${userId}: ${utils.getErrorMessage(
          err,
        )}`,
      );
    },
...
async function deleteProduct(productId: number) {
  return await ProductModel.destroy({ where: { id: productId } }).catch(
    (err: unknown) => {
      logger.error(
        `Could not perform soft delete for the product ${productId}: ${utils.getErrorMessage(
          err,
        )}`,
      );
    },
              ...
              await Promise.all(
                reviews.map(({ text, author }) =>
                  reviewsCollection
                    ...
                    .catch((err: unknown) => {
                      logger.error(
                        `Could not insert Product Review ${text}: ${utils.getErrorMessage(
                          err,
                        )}`,
                      );
                    }),
  ...
  return await Promise.all(
    baskets.map(async (basket) => {
      return await BasketModel.create({
        ...
        logger.error(
          `Could not insert Basket for UserId ${
            basket.UserId
          }: ${utils.getErrorMessage(err)}`,
        );
      });
  ...
  return await Promise.all(
    basketItems.map(async (basketItem) => {
      return await BasketItemModel.create(basketItem).catch((err: unknown) => {
        logger.error(
          `Could not insert BasketItem for BasketId ${
            basketItem.BasketId
          }: ${utils.getErrorMessage(err)}`,
        );
      });
...
async function createFeedback(
  ...
  return await FeedbackModel.create({
    ...
    logger.error(
      `Could not insert Feedback ${authoredComment} mapped to UserId ${UserId}: ${utils.getErrorMessage(
        err,
      )}`,
    );
  });
  ...
  await Promise.all(
    questions.map(async ({ question }) => {
      try {
        ...
        logger.error(
          `Could not insert SecurityQuestion ${question}: ${utils.getErrorMessage(
            err,
          )}`,
        );
      }
...
async function createSecurityAnswer(
  ...
  return await SecurityAnswerModel.create({
    ...
    logger.error(
      `Could not insert SecurityAnswer ${answer} mapped to UserId ${UserId}: ${utils.getErrorMessage(
        err,
      )}`,
    );
  });
    ...
    orders.map(
      ({ orderId, email, totalPrice, bonus, products, eta, delivered }) =>
        ordersCollection
          ...
          .catch((err: unknown) => {
            logger.error(
              `Could not insert Order ${orderId}: ${utils.getErrorMessage(
                err,
              )}`,
            );
          }),
