...
exports.solve = function (challenge) {
  ...
  challenge.save().success(function () {
    const pureDescription = sanitizeHtml(challenge.description, {
      allowedTags: [],
      allowedAttributes: []
    })
    console.log(colors.green('Solved') + ' challenge ' + colors.cyan(challenge.name) + ' (' + pureDescription + ')')
    if (global.io) {
      challenge.description = pureDescription
      global.io.emit('challenge solved', {challenge: challenge})
    }
  })
}
