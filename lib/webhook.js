...
exports.notify = (challenge, webhook = process.env.SOLUTIONS_WEBHOOK) => {
  si.get({ system: 'model, manufacturer', osInfo: 'distro, hostname', dockerInfo: 'id' }).then(sysInfo => {
    request.post(webhook, {
      json: {
        ...
        issuer: {
          device: `${sysInfo.system.model} (${sysInfo.system.manufacturer})`,
          os: sysInfo.osInfo.distro,
          hostName: sysInfo.osInfo.hostname,
          docker: sysInfo.dockerInfo.id,
