  ...
  if (config.has("application.easterEggPlanet.overlayMap")) {
    ...
    if (utils.isUrl(overlay)) {
      ...
      await utils.downloadToFile(
        overlayPath,
        "frontend/dist/frontend/assets/private/" + overlay,
      );
    }
...
const replaceImagePath = (overlay: string) => {
  const textureDeclaration =
    'orangeTexture = THREE.ImageUtils.loadTexture("/assets/private/' +
    overlay +
    '");';
  replace({
    regex: /orangeTexture = .*;/,
...
const replaceThreeJsTitleTag = () => {
  const threeJsTitleTag =
    "<title>Welcome to Planet " +
    config.get<string>("application.easterEggPlanet.name") +
    "</title>";
  replace({
    regex: /<title>.*<\/title>/,
