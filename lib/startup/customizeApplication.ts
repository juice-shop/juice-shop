...
const customizeLogo = async () => {
  await retrieveCustomFile(
    "application.logo",
    "frontend/dist/frontend/assets/public/images",
  );
};

const customizeChatbotAvatar = async () => {
  const avatarImage = await retrieveCustomFile(
    "application.chatBot.avatar",
    "frontend/dist/frontend/assets/public/images",
  );
  fs.copyFileSync(
    "frontend/dist/frontend/assets/public/images/" + avatarImage,
    "frontend/dist/frontend/assets/public/images/ChatbotAvatar.png",
  );
};
...
const customizeHackingInstructorAvatar = async () => {
  ...
  fs.copyFileSync(
    "frontend/dist/frontend/assets/public/images/" + avatarImage,
    "frontend/dist/frontend/assets/public/images/hackingInstructor.png",
  );
};

const customizeFavicon = async () => {
  const favicon = await retrieveCustomFile(
    "application.favicon",
    "frontend/dist/frontend/assets/public",
  );
  replace({
...
const retrieveCustomFile = async (
...
const customizeTheme = () => {
  const bodyClass =
    ...
    config.get<string>("application.theme") +
    '-theme"';
  replace({
...
const customizeCookieConsentBanner = () => {
  const contentProperty =
    '"content": { "message": "' +
    config.get<string>("application.cookieConsent.message") +
    '", "dismiss": "' +
    config.get<string>("application.cookieConsent.dismissText") +
    '", "link": "' +
    config.get<string>("application.cookieConsent.linkText") +
    '", "href": "' +
    config.get<string>("application.cookieConsent.linkUrl") +
    '" }';
  replace({
    regex:
      /"content": { "message": ".*", "dismiss": ".*", "link": ".*", "href": ".*" }/,
