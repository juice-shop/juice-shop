...
const validatePreconditions = async ({ exitOnFailure = true } = {}) => {
  ...
  success = checkIfRequiredFileExists('../../frontend/dist/frontend/main.js') && success
  success = checkIfRequiredFileExists('../../frontend/dist/frontend/polyfills.js') && success
  success = checkIfRequiredFileExists('../../frontend/dist/frontend/runtime.js') && success
  success = checkIfRequiredFileExists('../../frontend/dist/frontend/vendor.js') && success
...
const checkIfRequiredFileExists = (filePath) => {
  if (!fs.existsSync(path.resolve(__dirname, filePath))) {
    logger.warn(`Required file ${colors.bold(filePath)} is missing (${colors.red('NOT OK')})`)
    return false
  }
  logger.info(`Required file ${colors.bold(filePath)} is present (${colors.green('OK')})`)
  return true
}
