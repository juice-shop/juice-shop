...
const validateConfig = ({ products = config.get('products') } = {}) => {
  try {
  ...
  return products.every(
    (product) => {
      ...
      if (appliedSpeicals.length > 1) {
        throw new Error(`You cannot use Product ${product.name} for multiple Challenges.\nIt was attempted to be used for as ${appliedSpeicals.map(({ name }) => name).join(', ')}`)
      }
      return true
    }
  )
