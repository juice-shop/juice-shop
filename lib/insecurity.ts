...
export const hmac = (data: string) => crypto.createHmac('sha256', process.env.HMAC_SECRET || 'please-set-HMAC_SECRET').update(data).digest('hex')
...
export const authorize = (user = {}) => jwt.sign(user, process.env.JWT_PRIVATE_KEY || privateKey, { expiresIn: '6h', algorithm: 'RS256' })
export const verify = (token: string) => token ? (jws.verify as ((token: string, secret: string) => boolean))(token, publicKey) : false
export const decode = (token: string) => { return jws.decode(token)?.payload }

export const sanitizeHtml = (html: string) => sanitizeHtmlLib(html)
export const sanitizeLegacy = (input = '') => input.replace(/<[^>]*>/g, '')
