...
const privateKey = process.env.JWT_RSA_PRIVATE_KEY || fs.readFileSync('encryptionkeys/jwt.key', 'utf8')
...
export const isRedirectAllowed = (url: string) => {
  ...
  for (const allowedUrl of redirectAllowlist) {
    allowed = allowed || url === allowedUrl || url.startsWith(allowedUrl + '/') // vuln-code-snippet vuln-line redirectChallenge
  }
  return allowed
}
