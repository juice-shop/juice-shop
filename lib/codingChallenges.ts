...
function getCodeChallengesFromFile(file: FileMatch) {
  ...
  const challengeKeyRegex =
    /[/#]{0,2} vuln-code-snippet start (?<challenges>.*)/g;
  const challenges = [...fileContent.matchAll(challengeKeyRegex)]
    .flatMap((match) => match.groups?.challenges?.split(" ") ?? [])
    .filter(Boolean);

  return challenges.map((challengeKey) =>
    getCodingChallengeFromFileContent(fileContent, challengeKey),
  );
}

function getCodingChallengeFromFileContent(
  source: string,
  challengeKey: string,
) {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (
      new RegExp(`vuln-code-snippet vuln-line.*${challengeKey}`).exec(
        lines[i],
      ) != null
    ) {
      vulnLines.push(i + 1);
    } else if (
      new RegExp(`vuln-code-snippet neutral-line.*${challengeKey}`).exec(
        lines[i],
      ) != null
    ) {
...
export async function getCodeChallenges(): Promise<
  Map<string, CachedCodeChallenge>
> {
  if (_internalCodeChallenges === null) {
    _internalCodeChallenges = new Map<string, CachedCodeChallenge>();
    const filesWithCodeChallenges =
      await findFilesWithCodeChallenges(SNIPPET_PATHS);
