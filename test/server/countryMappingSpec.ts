...
describe("countryMapping", () => {
  ...
  it("should return configured country mappings", () => {
    countryMapping({
      get: sinon.stub().withArgs("ctf.countryMapping").returns("TEST"),
    } as unknown as IConfig)(req, res);
  ...
  it("should return server error when configuration has no country mappings", () => {
    countryMapping({
      get: sinon.stub().withArgs("ctf.countryMapping").returns(null),
    } as unknown as IConfig)(req, res);
  ...
  it(
    "should return " +
      (config.get("ctf.countryMapping") ? "no " : "") +
      "server error for active configuration from config/" +
