...
describe("utils", () => {
  describe("toSimpleIpAddress", () => {
    it("returns ipv6 address unchanged", () => {
      expect(
        utils.toSimpleIpAddress("2001:0db8:85a3:0000:0000:8a2e:0370:7334"),
      ).to.equal("2001:0db8:85a3:0000:0000:8a2e:0370:7334");
    });

    it("returns ipv4 address fully specified as ipv6 unchanged", () => {
      expect(utils.toSimpleIpAddress("0:0:0:0:0:ffff:7f00:1")).to.equal(
    ...
    it("returns ipv4-mapped address as ipv4 address", () => {
      expect(utils.toSimpleIpAddress("::ffff:192.0.2.128")).to.equal(
        "192.0.2.128",
      );
    });
  });
  ...
  describe("extractFilename", () => {
    ...
    it("returns filename from http:// URL", () => {
      expect(utils.extractFilename("http://bla.blubb/test.exe")).to.equal(
        "test.exe",
      );
    });
    ...
    it("ignores query part of http:// URL", () => {
      expect(
        utils.extractFilename("http://bla.blubb/test.exe?bla=blubb&a=b"),
      ).to.equal("test.exe");
    });

    it("also works for file:// URLs", () => {
      expect(utils.extractFilename("file:///C//Bla/Blubb/test.exe")).to.equal(
        "test.exe",
      );
    });
  });
    ...
    it("passes on Arch Linux passwd file content", () => {
      expect(
        utils.matchesEtcPasswdFile(
          "test:x:0:0:test:/test:/usr/bin/zsh\n" +
  ...
  describe("utils.getChallengeEnablementStatus", () => {
    ...
    for (const safetyMode of ["enabled", "disabled", "auto"] as const) {
      it(`challenges without disabledEnv are enabled with safetyMode set to ${safetyMode}`, () => {
        const challenge: ChallengeModel = {
          disabledEnv: null,
        } as unknown as ChallengeModel;

        expect(
          utils.getChallengeEnablementStatus(
            challenge,
            safetyMode,
            defaultIsEnvironmentFunctions,
          ),
    ...
    for (const testCase of testCases) {
      it(`safetyMode: 'enabled': challenge with disabledOnEnv ${testCase.name} should be marked as disabled`, () => {
        const challenge: ChallengeModel = {
          disabledEnv: testCase.name,
        } as unknown as ChallengeModel;

        const isEnvironmentFunctions = {
          ...defaultIsEnvironmentFunctions,
          [testCase.environmentFunction]: () => true,
        };
        expect(
          utils.getChallengeEnablementStatus(
            challenge,
