...
describe("codingChallengeFixes", () => {
  ...
  it("should have a correct fix for each coding challenge", async () => {
    for (const challenge of codingChallenges) {
      ...
      expect(
        fixes.correct,
        `Coding challenge ${challenge} does not have a correct fix file`,
      ).to.be.greaterThan(-1);
    }
  });
  ...
  it("should have a total of three or more fix options for each coding challenge", async () => {
    for (const challenge of codingChallenges) {
      ...
      expect(
        fixes.fixes.length,
        `Coding challenge ${challenge} does not have enough fix option files`,
  ...
  it("should have an info YAML file for each coding challenge", async () => {
    for (const challenge of codingChallenges) {
      expect(
        fs.existsSync("./data/static/codefixes/" + challenge + ".info.yml"),
        `Coding challenge ${challenge} does not have an info YAML file`,
      ).to.equal(true);
    }
  });
});
