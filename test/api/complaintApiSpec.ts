...
describe("/api/Complaints", () => {
  it("POST new complaint", () => {
    return frisby
      .post(API_URL + "/Complaints", {
        ...
        body: {
          message:
            "You have no clue what https://github.com/eslint/eslint-scope/issues/39 means, do you???",
        },
      })
      .expect("status", 201)
  ...
  it("GET all complaints is forbidden via public API", () => {
    return frisby.get(API_URL + "/Complaints").expect("status", 401);
  });

  it("GET all complaints", () => {
    return frisby
      .get(API_URL + "/Complaints", { headers: authHeader })
      .expect("status", 200);
  });
});
...
describe("/api/Complaints/:id", () => {
  it("GET existing complaint by id is forbidden", () => {
    return frisby
      .get(API_URL + "/Complaints/1", { headers: authHeader })
      .expect("status", 401);
  });

  it("PUT update existing complaint is forbidden", () => {
    return frisby
      .put(API_URL + "/Complaints/1", {
        headers: authHeader,
        body: {
  ...
  it("DELETE existing complaint is forbidden", () => {
    return frisby
      .del(API_URL + "/Complaints/1", { headers: authHeader })
      .expect("status", 401);
  });
});
