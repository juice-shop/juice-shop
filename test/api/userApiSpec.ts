...
describe("/api/Users", () => {
  it("GET all users is forbidden via public API", () => {
    return frisby.get(`${API_URL}/Users`).expect("status", 401);
  });
...
describe("/api/Users/:id", () => {
  it("GET existing user by id is forbidden via public API", () => {
    return frisby.get(`${API_URL}/Users/1`).expect("status", 401);
  });
  ...
  it("DELETE existing user is forbidden via public API", () => {
    return frisby.del(`${API_URL}/Users/1`).expect("status", 401);
  });

  it("GET existing user by id", () => {
    return frisby
      .get(`${API_URL}/Users/1`, { headers: authHeader })
      .expect("status", 200);
  });
  ...
  it("DELETE existing user is forbidden via API even when authenticated", () => {
    return frisby
      .del(`${API_URL}/Users/1`, { headers: authHeader })
      .expect("status", 401);
  });
});
...
describe("/rest/user/whoami", () => {
  ...
  it("GET who-am-i request returns nothing on broken auth token", () => {
    return frisby
      .get(`${REST_URL}/user/whoami`, {
        headers: { Authorization: "BoarBeatsBear" },
      })
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
      .expect("json", {
  ...
  it("GET who-am-i request returns nothing on expired auth token", () => {
    return frisby
      .get(`${REST_URL}/user/whoami`, {
        headers: {
          Authorization:
            "eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCJ9..FL0kkcInY5sDMGKeLHfEOYDTQd3BjR6_mK7Tcm_RH6iCLotTSRRoRxHpLkbtIQKqBFIt14J4BpLapkzG7ppRWcEley5nego-4iFOmXQvCBz5ISS3HdtM0saJnOe0agyVUen3huFp4F2UCth_y2ScjMn_4AgW66cz8NSFPRVpC8g",
        },
      })
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
