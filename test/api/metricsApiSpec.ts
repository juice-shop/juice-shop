...
describe("/metrics", () => {
  xit("GET metrics via public API that are available instantaneously", () => {
    // FIXME Flaky on CI/CD on at least Windows
    return frisby
      .get(API_URL)
      .expect("status", 200)
      ...
      .expect(
        "bodyContains",
        /^.*_version_info{version="[0-9]+.[0-9]+.[0-9]+(-SNAPSHOT)?",major="[0-9]+",minor="[0-9]+",patch="[0-9]+",app=".*"} 1$/gm,
      )
      .expect(
        "bodyContains",
      ...
      .expect(
        "bodyContains",
        /^.*_users_registered{type="standard",app=".*"} [0-9]*$/gm,
      )
      .expect(
        "bodyContains",
        /^.*_users_registered{type="deluxe",app=".*"} [0-9]*$/gm,
      )
      .expect("bodyContains", /^.*_users_registered_total{app=".*"} [0-9]*$/gm)
      .expect("bodyContains", /^.*_wallet_balance_total{app=".*"} [0-9]*$/gm)
      .expect(
        "bodyContains",
        /^.*_user_social_interactions{type="review",app=".*"} [0-9]*$/gm,
      )
      .expect(
        "bodyContains",
        /^.*_user_social_interactions{type="feedback",app=".*"} [0-9]*$/gm,
      )
      .expect(
        "bodyContains",
        /^.*_user_social_interactions{type="complaint",app=".*"} [0-9]*$/gm,
      )
      .expect(
        "bodyContains",
        /^http_requests_count{status_code="[0-9]XX",app=".*"} [0-9]*$/gm,
      );
  });
  ...
  xit("GET file upload metrics via public API", () => {
    ...
    const file = path.resolve(
      __dirname,
      "../files/validSizeAndTypeForClient.pdf",
    );
    ...
    return frisby
      ...
      .then(() => {
        return frisby
          ...
          .expect(
            "bodyContains",
            /^file_uploads_count{file_type=".*",app=".*"} [0-9]*$/gm,
          );
      });
  });
    ...
    return frisby
      ...
      .then(() => {
        return frisby
          ...
          .expect(
            "bodyContains",
            /^file_upload_errors{file_type=".*",app=".*"} [0-9]*$/gm,
          );
      });
  });
});
