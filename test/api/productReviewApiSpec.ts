...
describe("/rest/products/:id/reviews", () => {
  ...
  it("GET product reviews by product id", () => {
    return frisby
      .get(`${REST_URL}/products/1/reviews`)
      .expect("status", 200)
  ...
  it("GET product reviews attack by injecting a mongoDB sleep command", () => {
    return frisby
      .get(`${REST_URL}/products/sleep(1)/reviews`)
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
      .expect("jsonTypes", reviewResponseSchema);
  });
  ...
  xit("GET product reviews by alphanumeric non-mongoDB-command product id", () => {
    ...
    return frisby
      .get(`${REST_URL}/products/kaboom/reviews`)
      .expect("status", 400);
  });

  it("PUT single product review can be created", () => {
    return frisby
      .put(`${REST_URL}/products/1/reviews`, {
        body: {
          message: "Lorem Ipsum",
          author: "Anonymous",
        },
...
describe("/rest/products/reviews", () => {
  ...
  it("PATCH single product review can be edited", () => {
    return frisby
      .patch(`${REST_URL}/products/reviews`, {
        headers: authHeader,
        body: {
          id: reviewId,
          message: "Lorem Ipsum",
  ...
  it("POST non-existing product review cannot be liked", () => {
    return frisby
      ...
      .then(({ json: jsonLogin }) => {
        return frisby
          .post(`${REST_URL}/products/reviews`, {
            headers: {
  ...
  it("PATCH multiple product review via injection", () => {
    ...
    const totalReviews = config
      .get<Product[]>("products")
      .reduce((sum: number, { reviews = [] }: any) => sum + reviews.length, 1);
