...
describe("/rest/basket/:id", () => {
  it("GET existing basket by id is not allowed via public API", () => {
    return frisby.get(REST_URL + "/basket/1").expect("status", 401);
  });

  it("GET empty basket when requesting non-existing basket id", () => {
    return frisby
      .get(REST_URL + "/basket/4711", { headers: authHeader })
      .expect("status", 200)
  ...
  it("GET existing basket with contained products by id", () => {
    return frisby
      .get(REST_URL + "/basket/1", { headers: authHeader })
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
...
describe("/api/Baskets", () => {
  ...
  it("GET all baskets is not part of API", () => {
    return frisby
      .get(API_URL + "/Baskets", { headers: authHeader })
      .expect("status", 500);
  });
});
...
describe("/api/Baskets/:id", () => {
  it("GET existing basket is not part of API", () => {
    return frisby
      .get(API_URL + "/Baskets/1", { headers: authHeader })
      .expect("status", 500);
  });
  ...
  it("DELETE existing basket is not part of API", () => {
    return frisby
      .del(API_URL + "/Baskets/1", { headers: authHeader })
      .expect("status", 500);
  });
});
...
describe("/rest/basket/:id/checkout", () => {
  it("POST placing an order for a basket is not allowed via public API", () => {
    return frisby.post(REST_URL + "/basket/1/checkout").expect("status", 401);
  });

  it("POST placing an order for an existing basket returns orderId", () => {
    return frisby
      .post(REST_URL + "/basket/1/checkout", { headers: authHeader })
      .expect("status", 200)
      .then(({ json }) => {
  ...
  it("POST placing an order for a non-existing basket fails", () => {
    return frisby
      .post(REST_URL + "/basket/42/checkout", { headers: authHeader })
      .expect("status", 500)
  ...
  it("POST placing an order for a basket with a negative total cost is possible", () => {
    return frisby
      ...
      .then(() => {
        return frisby
          .post(REST_URL + "/basket/3/checkout", { headers: authHeader })
          .expect("status", 200)
          .then(({ json }) => {
            expect(json.orderConfirmation).toBeDefined();
          });
  ...
  it("POST placing an order for a basket with 99% discount is possible", () => {
    return frisby
      ...
      .then(() => {
        return frisby
          .post(REST_URL + "/basket/2/checkout", { headers: authHeader })
          .expect("status", 200)
          .then(({ json }) => {
            expect(json.orderConfirmation).toBeDefined();
          });
...
describe("/rest/basket/:id/coupon/:coupon", () => {
  ...
  it("PUT apply invalid coupon is not accepted", () => {
    return frisby
      .put(REST_URL + "/basket/1/coupon/xxxxxxxxxx", { headers: authHeader })
      .expect("status", 404);
  });
  ...
  it("PUT apply outdated coupon is not accepted", () => {
    return frisby
      .put(
        REST_URL + "/basket/1/coupon/" + encodeURIComponent(outdatedCoupon),
        { headers: authHeader },
      )
      .expect("status", 404);
  });
  ...
  it("PUT apply valid coupon to non-existing basket throws error", () => {
    return frisby
      .put(
        REST_URL + "/basket/4711/coupon/" + encodeURIComponent(validCoupon),
        { headers: authHeader },
      )
      .expect("status", 500);
  });
