...
describe("/api/Addresss", () => {
  it("GET all addresses is forbidden via public API", () => {
    return frisby.get(API_URL + "/Addresss").expect("status", 401);
  });

  it("GET all addresses", () => {
    return frisby
      .get(API_URL + "/Addresss", { headers: authHeader })
      .expect("status", 200);
  });

  it("POST new address with all valid fields", () => {
    return frisby
      .post(API_URL + "/Addresss", {
        headers: authHeader,
        body: {
          fullName: "Jim",
...
describe("/api/Addresss/:id", () => {
  ...
  it("GET address by id is forbidden via public API", () => {
    return frisby.get(API_URL + "/Addresss/" + addressId).expect("status", 401);
  });

  it("PUT update address is forbidden via public API", () => {
    return frisby
      .put(
        API_URL + "/Addresss/" + addressId,
        {
          quantity: 2,
        },
        { json: true },
  ...
  it("DELETE address by id is forbidden via public API", () => {
    return frisby.del(API_URL + "/Addresss/" + addressId).expect("status", 401);
  });

  it("GET address by id", () => {
    return frisby
      .get(API_URL + "/Addresss/" + addressId, { headers: authHeader })
      .expect("status", 200);
  });
  ...
  it("DELETE address by id", () => {
    return frisby
      .del(API_URL + "/Addresss/" + addressId, { headers: authHeader })
      .expect("status", 200);
  });
});
