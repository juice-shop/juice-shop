...
describe("/rest/user/data-export", () => {
          ...
          .then(() => {
            return frisby
              ...
              .then(({ json }) => {
                ...
                expect(parsedData.orders[0].products[0].name).toBe(
      ...
      .then(({ json: jsonLogin }) => {
        return frisby
          ...
          .then(({ json }) => {
            ...
            expect(parsedData.reviews[1].message).toBe(
              "Fresh out of a replicator.",
            );
            expect(parsedData.reviews[1].author).toBe(
              "jim@" + config.get<string>("application.domain"),
            );
  ...
  it("Export data including memories without use of CAPTCHA", () => {
    ...
    form.append(
      "image",
      fs.createReadStream(file) as unknown as Blob,
      "Valid Image",
    ); // casting to blob as the frisby types are wrong and wont accept the fileStream type 'Valid Image')
          ...
          .then(() => {
            return frisby
              ...
              .then(({ json }) => {
                ...
                expect(parsedData.memories[0].imageUrl).toContain(
                  "assets/public/images/uploads/valid-image",
                );
              });
          });
      });
