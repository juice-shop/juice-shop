      ...
      .then(({ json }) => {
        return frisby
          .post(API_URL + "/Feedbacks", {
            ...
            body: {
              comment:
                'I am a harm<script>steal-cookie</script><img src="csrf-attack"/><iframe src="evil-content"></iframe>less comment.',
              rating: 1,
      ...
      return frisby
        ...
        .then(({ json }) => {
          return frisby
            ...
            .expect("json", "data", {
              comment:
                'The sanitize-html module up to at least version 1.4.2 has this issue: <iframe src="javascript:alert(`xss`)">',
            });
        });
    });
  }
      ...
      .then(({ json }) => {
        return frisby
          .post(API_URL + "/Feedbacks", {
            ...
            body: {
              comment:
                "Pickle Rick says your express-jwt 0.1.3 has Eurogium Edule and Hueteroneel in it!",
              rating: 0,
              UserId: 4711,
              captchaId: json.captchaId,
              captcha: json.answer,
          ...
          .then(({ json }) => {
            return frisby
              .post(API_URL + "/Feedbacks", {
                ...
                body: {
                  comment:
                    "Stupid JWT secret and being typosquatted by epilogue-js and ngy-cookie!",
                  rating: 5,
                  UserId: 4,
                  captchaId: json.captchaId,
                  captcha: json.answer,
      ...
      .then(({ json }) => {
        return frisby
          ...
          .then(({ json }) => {
            expect(
              json.message.match(
                /notNull Violation: (Feedback\.)?rating cannot be null/,
              ),
            );
          });
      });
...
describe("/api/Feedbacks/:id", () => {
  ...
  it("GET existing feedback by id", () => {
    return frisby
      .get(API_URL + "/Feedbacks/1", { headers: authHeader })
      .expect("status", 200);
  });
  ...
  it("DELETE existing feedback is forbidden via public API", () => {
    return frisby.del(API_URL + "/Feedbacks/1").expect("status", 401);
  });
    ...
    return frisby
      ...
      .then(({ json }) => {
        return frisby
          ...
          .then(({ json }) => {
            return frisby
              .del(API_URL + "/Feedbacks/" + json.data.id, {
                headers: authHeader,
              })
              .expect("status", 200);
          });
