        ...
        return frisby
          ...
          .then(({ json }) => {
            return frisby
              .get(
                REST_URL +
                  "/user/change-password?current=kunigunde&new=foo&repeat=foo",
                {
                  headers: {
    ...
    return frisby
      ...
      .then(({ json }) => {
        return frisby
          .get(
            REST_URL +
              "/user/change-password?current=definetely_wrong&new=blubb&repeat=blubb",
            {
    ...
    return frisby
      ...
      .then(({ json }) => {
        return frisby
          .get(
            REST_URL +
              "/user/change-password?new=slurmCl4ssic&repeat=slurmCl4ssic",
            {
              headers: { Authorization: "Bearer " + json.authentication.token },
            },
          )
