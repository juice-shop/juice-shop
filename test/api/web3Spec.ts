...
describe("/submitKey", () => {
  ...
  it("POST public wallet key in request body gets rejected as such", () => {
    return frisby
      .post(REST_URL + "/submitKey", {
        privateKey:
          "0x02c7a2a93289c9fbda5990bac6596993e9bb0a8d3f178175a80b7cfd983983f506",
      })
      .expect("status", 401)
      .expect("header", "content-type", /application\/json/)
      .expect("json", {
...
describe("/walletExploitAddress", () => {
  it("POST missing wallet address in request body still leads to success notification", () => {
    return frisby
      .post(REST_URL + "/walletExploitAddress")
      .expect("status", 200)
      .expect("header", "content-type", /application\/json/)
      .expect("json", {
        success: true,
