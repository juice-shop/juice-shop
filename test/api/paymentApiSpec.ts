...
describe("/api/Cards", () => {
  it("GET all cards is forbidden via public API", () => {
    return frisby.get(API_URL + "/Cards").expect("status", 401);
  });

  it("GET all cards", () => {
    return frisby
      .get(API_URL + "/Cards", { headers: authHeader })
      .expect("status", 200);
  });
...
describe("/api/Cards/:id", () => {
  ...
  it("GET card by id is forbidden via public API", () => {
    return frisby.get(API_URL + "/Cards/" + cardId).expect("status", 401);
  });
  ...
  it("DELETE card by id is forbidden via public API", () => {
    return frisby.del(API_URL + "/Cards/" + cardId).expect("status", 401);
  });

  it("GET card by id", () => {
    return frisby
      .get(API_URL + "/Cards/" + cardId, { headers: authHeader })
      .expect("status", 200);
  });
  ...
  it("DELETE card by id", () => {
    return frisby
      .del(API_URL + "/Cards/" + cardId, { headers: authHeader })
      .expect("status", 200);
  });
});
