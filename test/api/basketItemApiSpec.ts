...
describe("/api/BasketItems", () => {
  ...
  it("GET all basket items", () => {
    return frisby
      .get(API_URL + "/BasketItems", { headers: authHeader })
      .expect("status", 200);
  });
...
describe("/api/BasketItems/:id", () => {
  ...
  it("GET newly created basket item by id", () => {
    return frisby
      ...
      .then(({ json }) => {
        return frisby
          .get(API_URL + "/BasketItems/" + json.data.id, {
            headers: authHeader,
          })
          .expect("status", 200);
      });
  ...
  it("PUT update newly created basket item", () => {
    return frisby
      ...
      .then(({ json }) => {
        return frisby
          .put(API_URL + "/BasketItems/" + json.data.id, {
            headers: authHeader,
            body: {
              quantity: 20,
            },
    ...
    return frisby
      ...
      .then(({ json }) => {
        return frisby
          ...
          .expect("json", {
            message:
              "null: `BasketId` cannot be updated due `noUpdate` constraint",
            errors: [
              {
                field: "BasketId",
                message:
                  "`BasketId` cannot be updated due `noUpdate` constraint",
              },
            ],
          });
      });
    ...
    return frisby
      ...
      .then(({ json }) => {
        return frisby
          ...
          .expect("json", {
            message:
              "null: `ProductId` cannot be updated due `noUpdate` constraint",
            errors: [
              {
                field: "ProductId",
                message:
  ...
  it("DELETE newly created basket item", () => {
    return frisby
      ...
      .then(({ json }) => {
        return frisby
          .del(API_URL + "/BasketItems/" + json.data.id, {
            headers: authHeader,
          })
          .expect("status", 200);
      });
