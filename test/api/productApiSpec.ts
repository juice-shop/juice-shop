...
const tamperingProductId =
  config
    ...
    .findIndex((product) => !!product.urlForProductTamperingChallenge) + 1;
...
describe("/api/Products/:id", () => {
  ...
  it("DELETE existing product is forbidden via public API", () => {
    return frisby.del(API_URL + "/Products/1").expect("status", 401);
  });

  it("DELETE existing product is forbidden via API even when authenticated", () => {
    return frisby
      .del(API_URL + "/Products/1", { headers: authHeader })
      .expect("status", 401);
  });
});
