...
describe("/api/PrivacyRequests", () => {
  ...
  it("GET all privacy requests is forbidden via public API", () => {
    return frisby.get(API_URL + "/PrivacyRequests").expect("status", 401);
  });
});
...
describe("/api/PrivacyRequests/:id", () => {
  it("GET all privacy requests is forbidden", () => {
    return frisby
      .get(API_URL + "/PrivacyRequests", { headers: authHeader })
      .expect("status", 401);
  });

  it("GET existing privacy request by id is forbidden", () => {
    return frisby
      .get(API_URL + "/PrivacyRequests/1", { headers: authHeader })
      .expect("status", 401);
  });

  it("PUT update existing privacy request is forbidden", () => {
    return frisby
      .put(API_URL + "/PrivacyRequests/1", {
        headers: authHeader,
        body: {
  ...
  it("DELETE existing privacy request is forbidden", () => {
    return frisby
      .del(API_URL + "/PrivacyRequests/1", { headers: authHeader })
      .expect("status", 401);
  });
});
