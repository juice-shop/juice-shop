# Cursor Instructions for OWASP Juice Shop

This file provides guidelines for using Cursor IDE when contributing to OWASP Juice Shop.

## Primary Reference

For comprehensive guidelines on using AI assistants (including Cursor) with this project, please refer to [CLAUDE.md](../CLAUDE.md). That document covers:
...
**All contributors using Cursor must follow the guidelines in [CLAUDE.md](../CLAUDE.md) before submitting pull requests.**
...
Before suggesting code or accepting Cursor suggestions, ensure:
...
**Code Generation**: Use Cursor's multi-line completion and edit mode, but always verify generated code follows project patterns.

**Creating Tests**: Generate unit tests and E2E tests, but verify they pass locally and align with existing test patterns.

**Code Refactoring**: Use Cursor to improve code quality, but verify changes don't break functionality and pass RSN if applicable.

**Bug Fixes**: Leverage Cursor to identify and fix issues, but ensure fixes are tested thoroughly.

**Documentation**: Use Cursor for clear comments and documentation, but remove verbose explanations (see CLAUDE.md guidelines).

## Quick Checklist for Cursor-Assisted Work
...
Remember: Cursor is a productivity tool. You are responsible for the quality, correctness, and security of your contributions.
