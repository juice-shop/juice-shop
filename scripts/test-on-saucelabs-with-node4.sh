...
if [ "$TRAVIS_NODE_VERSION" == "4" ]; then
    if [ "$TRAVIS_PULL_REQUEST" == "false" ] || [ "$TRAVIS_BRANCH" != "develop" ] && [ "$TRAVIS_PULL_REQUEST" != "false" ]; then
        echo "Running unit tests on SauceLabs..."
        karma start karma.conf-ci.js
        echo "Running e2e tests on SauceLabs..."
        npm run protractor
    else
        echo "Skipping SauceLabs tests for PR #$TRAVIS_PULL_REQUEST onto develop branch (only executed for PRs onto master)"
    fi
  else
