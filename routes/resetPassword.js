...
function verifySecurityAnswerChallenges (user, answer) {
  ...
  utils.solveIf(challenges.geoStalkingMetaChallenge, () => { return user.id === users.GeoStalking1.id && answer === config.get('challenges.geoStalking.securityAnswerEasy') })
  utils.solveIf(challenges.geoStalkingVisualChallenge, () => { return user.id === users.GeoStalking2.id && answer === config.get('challenges.geoStalking.securityAnswerHard') })
}
