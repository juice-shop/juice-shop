...
export const status = function status () {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      res.status(200).json({
        status: bot.training.state,
        body: bot.training.state ? security.sanitizeHtml(bot.greet(`${user.id}`)) : `${security.sanitizeHtml(config.get<string>('application.chatBot.name'))} isn't ready at the moment, please wait while I set things up`
      })
    } catch (err) {
      next(new Error('Blocked illegal activity by ' + req.socket.remoteAddress))
    }
