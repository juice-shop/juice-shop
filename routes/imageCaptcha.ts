...
export function imageCaptchas() {
  return (req: Request, res: Response) => {
    ...
    if (!user) {
      res
        .status(401)
        .send(res.__("You need to be logged in to request a CAPTCHA."));
      return;
    }
    ...
    imageCaptchaInstance
      ...
      .catch(() => {
        res
          .status(400)
          .send(res.__("Unable to create CAPTCHA. Please try again."));
      });
  };
}

export const verifyImageCaptcha =
    ...
    ImageCaptchaModel.findAll({
      ...
      .catch(() => {
        res
          ...
          .send(
            res.__(
              "Something went wrong while submitting CAPTCHA. Please try again.",
            ),
          );
      });
  };
