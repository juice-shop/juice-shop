...
> The packaged distribution will not work on other node.js versions because SQLite is compiled from source for the OS and node.js version which ```npm install``` is executed on.
